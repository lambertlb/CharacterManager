...
class PersonalInfo(CharacterEntity):
    ...
    def register(self):
        ...
        if not self._deityScript and self.Deity != '':
            self._deityScript = Entity.instanceFromScript(
                'CharacterTemplates.scripts.Deities.' + self.Deity)
        if self.Deity != '':
            self._deityScript.register()
        if not self._raceScript and self.Race != '':
            self._raceScript = Entity.instanceFromScript(
                'CharacterTemplates.scripts.Races.' + self.Race)
        if self.Race != '':
    ...
    def getPropertyDataForDeity(self, propertyName, propertyType):
        templatePath = Services.getConfigurationManager().getValue(CharacterManagerConfig.sourcesKey, CharacterManagerConfig.characterTemplateDirectoryKey)
