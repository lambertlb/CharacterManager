...
class Entity:
			...
			if inspect.isclass(item) and issubclass(item, filter):
				if name != filter.__name__:
					if not classes.get(name):
						if module.__name__ == item.__module__:
							classes[name] = Entity.createInstanceFromClass(
								getattr(module, name))
	...
	def setupJsonTypeHandlers():
		...
		Entity.jsonHandlers.stringHandler = lambda propertyName, definition, data, entity: setattr(entity, propertyName, data)
		Entity.jsonHandlers.integerHandler = lambda propertyName, definition, data, entity: setattr(entity, propertyName, int(data))
		Entity.jsonHandlers.numberHandler = lambda propertyName, definition, data, entity: setattr(entity, propertyName, float(data))
		Entity.jsonHandlers.booleanHandler = lambda propertyName, definition, data, entity: setattr(entity, propertyName, data)
		Entity.jsonHandlers.nullHandler = lambda propertyName, definition, data, entity: setattr(entity, propertyName, None)
		Entity.jsonHandlers.arrayHandler = Entity.arrayTypeHandler
		Entity.jsonHandlers.objectHandler = Entity.objectTypeHandler
		assert Entity.jsonHandlers.stringHandler
		assert Entity.jsonHandlers.integerHandler
		assert Entity.jsonHandlers.numberHandler
		assert Entity.jsonHandlers.nullHandler
		assert Entity.jsonHandlers.arrayHandler
		assert Entity.jsonHandlers.objectHandler
		assert Entity.jsonHandlers.booleanHandler
		pass
	...
	def getPropertyData(entity, propertyName, data):
		...
		Entity.jsonHandlers.getHandler(dataType)(propertyName, definition, data, entity)
		pass
	...
	def addArrayData(array: list, definition: dict, data):
		...
		for element in data:
			...
			if isinstance(element, dict):  # adding Entity?
				dataObject = Entity.createEntityFromJsonData(
					element, definition['items'])
	...
	def setupJsonDataHandlers(self):
		...
		self._dataHandlers.stringHandler = lambda propertyName, definition, data, entity: setattr(self, propertyName, '')
		self._dataHandlers.integerHandler = lambda propertyName, definition, data, entity: setattr(self, propertyName, int(0))
		self._dataHandlers.numberHandler = lambda propertyName, definition, data, entity: setattr(self, propertyName, float(0.0))
		self._dataHandlers.booleanHandler = lambda propertyName, definition, data, entity: setattr(self, propertyName, False)
		self._dataHandlers.nullHandler = lambda propertyName, definition, data, entity: setattr(self, propertyName, None)
		self._dataHandlers.arrayHandler = lambda propertyName, definition, data, entity: setattr(self, propertyName, [])
		self._dataHandlers.objectHandler = lambda propertyName, definition, data, entity: setattr(self, propertyName, self.createFromTemplate(definition))
		pass

	def addNeededProperty(self, properties, propertyName):
		self.setupJsonDataHandlers()
		definition = properties.get(propertyName)
		neededType = definition.get('type')
		self._dataHandlers.getHandler(neededType)(propertyName, definition, None, None)
		pass
